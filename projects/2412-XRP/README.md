...
- Ripple的商业模式如何？该商业模式是否在XRP的估值逻辑中发挥作用？其团队和融资背景如何？
- XRPL相比其他公链有什么特点？目前的链上数据如何？XRPL值得期待的叙事和未来发展？
- XRP的经济模型、代币效用和估值水平如何？
- 最后，基于对前面问题的回答，探讨XRP的此次涨幅是否有持续的可能，面临哪些阻力？
...
|          | **12** | **将550亿个XRP分别放进55个Escrow的托管合约帐户中，每个帐户分别是10亿XRP，每个月的头一天解锁一个托管合约帐户，开放让企业购买，而当月未被购买的XRP，将会放回到新的智能合约。** |
...
Ripple的核心商业模式无疑是ToB的，产品主要面向企业级用户，其中仅按需支付ODL可能对C端个人有一定吸引力。

Ripple官网列出的产品包括：直接支付、按需支付、资产托管和稳定币四种。下面分别介绍它们。
...
ODL的用户界面基于点对点分布式应用程序RippleNet，它维护一个虚拟账本来模拟 Nostro 和 Vostro 账户和关系，由一个双向消息传递层和一个结算层组成。可以使用一些 API 操作将 RippleNet 与支付系统集成，使用 API 操作将资金复制到 RippleNet 虚拟账户，而不是访问金融机构的核心银行系统。RippleNet 部署在 Ripple 托管的云环境中，不需要客户维护。
...
解释二：如果认为市场是有效的，参与者都是理性的，那为什么XRP代币价格上涨呢？可能是Ripple官方给下文要介绍的XRPL公链及基金会等相关主体提供了一定的资金支持、人才支撑等等。这些支持虽说不太可能直接支撑起XRP的买盘，但是长远来看，这对XRPL公链的生态发展是一种利好，进而利好XRP代币。市场可能只是提前把这部分长远期的利好反映在了XRP代币的价格上面。

这两种解释都是有可能的。总之，虽说Ripple的核心业务逻辑确实没有直接赋能XRP价格上涨，但是却能够通过其他渠道发挥支撑作用，不要低估了Ripple利好消息对XRP价格的影响。
...
|      | 12   | 将550亿个XRP（总供应量的 55%）分别放进55个Escrow的托管合约帐户中，每个帐户分别是10亿XRP，在接下来的55个月内，每个月的头一天解锁一个托管合约帐户，开放让企业购买，而当月未被购买的XRP，将会放回到新的托管账户中。 |
...
值得指出的是，XRPL 有一个内置的CLOB DEX，与其他使用智能合约建立 DEX 的网络不同。原生 DEX 的好处是信任假设较少和流动性整合，而不是智能合约固有的漏洞。
...
[运行 XRP Ledger Validator](https://xrpl.org/blog/2020/running-an-xrp-ledger-validator)

[rippled 版本 1.0.0](https://xrpl.org/blog/2018/rippled-1.0.0)

[去中心化策略更新](https://xrpl.org/blog/2017/decent-strategy-update)

[XRP Ledger：不仅仅是瑞波币 |梅萨里](https://messari.io/report/xrp-ledger-more-than-ripples)

XRP Ledger 的主要优势之一是快速高效的共识算法，它可以在 4 到 5 秒内结算交易，同时以每秒高达 1,500 笔交易的吞吐量进行处理。
...
此外，与其他去中心化账本不同，XRPL不会通过运行验证者为共识过程做出贡献提供直接的经济激励。其他区块链提供直接激励，例如挖矿和质押或交易优势的奖励。相反，缺乏对 XRP Ledger 验证者的直接激励吸引了自然的利益相关者。
...
表：XRPL 链上活动半年度数据  数据来源：XPMarket  日期：2024年12月4日
...
- 通胀机制：无。意味着XRP代币的排放实质全由市场供需决定。其中Ripple的排放（包括托管锁定和即时可用的部分）尤其值得关注。
- 通缩机制：交易费用会被销毁。自网络启动以来，已销毁近 1300 万 XRP。销毁率较低。
...
2017年12月，Ripple将550亿个XRP分别放进55个Escrow的托管合约帐户中，每个帐户分别是10亿XRP，每个月的头一天解锁一个托管合约帐户，开放让企业购买，而当月未被购买的XRP，将会放回到新的智能合约。

从2023年Q1开始，Ripple在季度报告中会公布排放的XRP数量。Ripple 拥有的 XRP 分为两类：目前在其钱包中可用的 XRP，以及受账本托管锁定的 XRP，这些锁定将在未来 42 个月内每月释放。对于后一类，Ripple 无法直接访问该 XRP。
...
可以看出，在23Q1-24Q3这个时间段内，每季度Ripple抛售约800M（8亿）以上的XRP代币，价值几乎都在400M（4亿）美元以上。
